...
# actual total segments  
d3 <- d2 %>% 
  filter(num.from.base != 0 )
...
# remove extra missing segments (n = 23)
d3 <- d3 %>% 
  filter(!is.na(n15)) 

# remove pre-deployment segments 
d3 <- d3 %>% 
  group_by(seal) %>% 
  filter(date.est >= start.date) # n = 222

# remove glycine values and pre-deployment values
d3$n15[d3$has.glycine == TRUE ] <- NA 
...
h2.1 <- h2 %>% 
  filter(!is.na(x)) %>% 
  arrange(seal, num.from.tip) %>% 
  mutate( latitude = y * -1, z = jitter(z), group = as.factor(paste(year,state, sep = ':')))

lmc <- lmeControl(opt = 'optim')

## C13
gam1 <- gamm(c13+17 ~  s(latitude, bs = 'cr')  ,
             random = list(seal = ~1), corAR1(form = ~1|seal), data = h2.1, control = lmc, method = 'REML')
summary(gam1$gam)
acf(residuals(gam1$lme, type = 'normalized'))
par(mfrow=c(2,2))
gam.check(gam1$gam)
par(mfrow=c(1,1))


gam1.1 <- gamm(c13+17 ~ s(latitude, by = year) , control = lmc, weights = varPower(form = ~latitude),
             random = list(seal = ~1), correlation = corAR1(form = ~1), data = h2.1,  method = 'REML')
summary(gam1.1$gam)
acf(residuals(gam1.1$lme, type = 'normalized'))
plot.gam(gam1.1$gam, pages = 1, shade = T, seWithMean = T, shift = coef(gam1.1$gam)[1]-17)
par(mfrow=c(2,2))
gam.check(gam1.1$gam)
par(mfrow=c(1,1))


## N15
gam2 <- gamm(n15 ~  s(latitude) ,
             random = list(seal = ~1),  data = h2.1, control = lmc, method = 'REML')
summary(gam2$gam)
acf(residuals(gam1$lme, type = 'normalized'))
plot(gam2$gam, pages = 1, shade = T, seWithMean = T, shift = coef(gam2$gam)[1])
par(mfrow=c(2,2))
gam.check(gam2$gam)
par(mfrow=c(1,1))

gam2.1 <- gamm(n15 ~ s(latitude, by = year),  random = list(seal = ~1), data = h2.1, control = lmc, method = 'REML')
summary(gam2.1$gam) %>% tidy
acf(residuals(gam2.1$lme, type = 'normalized'))
plot(gam2.1$gam, pages = 1,  shade = T, seWithMean = T, shift = coef(gam2.1$gam)[1])
par(mfrow=c(2,2))
gam.check(gam2.1$gam)
par(mfrow=c(1,1))

AIC(gam2$lme, gam2.1$lme)

## Model effects plots 
pdf(file = './plots/SI_vs_latitude.pdf', height = 7, width = 7)
par(mfrow=c(2,1), mar=c(5,5,1,1))
plot(gam1.1$gam, select = 2, xlab = NA, shade = T, seWithMean = T, 
     shift = coef(gam1.1$gam)[1]-17,
     ylab =  expression(paste( delta ^{13}, "C ", "(‰)")))
# mtext(expression(paste("(a) ", delta ^"13", "C")), adj = 0, line = -2, at = c(44, 0.5))
plot(gam2.1$gam, select = 2,  shade = T, seWithMean = T, 
     shift = coef(gam2.1$gam)[1], xlab = 'Latitude south (degrees)',
     ylab =  expression(paste( delta ^{15}, "N ", "(‰)")))
# mtext(expression(paste("(b) ", delta ^"15", "N")), adj = 0, line = -2, at = c(44, 0.5))
dev.off()

## SAVE summary tables
tab <- tidy(gam1.1$gam) %>% 
  mutate(formula = as.character(gam1.1$gam$formula) %>% reduce(paste))
tab <- tidy(gam2.1$gam) %>% 
  mutate(formula = as.character(gam2.1$gam$formula) %>% reduce(paste)) %>% 
  bind_rows(tab) %>% 
  mutate_if(is.numeric, ~round(., 3))
# write.xlsx(tab, file="./output/Results_all.xlsx",
           # sheetName="GAMM-SUMMARY", append=FALSE)
...
AICtab <- function(..., models){
  tab1 <- ICtab(..., base = T, logLik = T, type = 'AIC') 
  model.names <- ICtab(...) %>% attributes() %>% .$row.names
  tab2 <- tab1 %>% 
    print() %>% 
    as.data.frame %>% 
    mutate(model = as.numeric(substr(model.names,3,3))) %>% 
    arrange(model)
  
  form <- purrr::map(models, function(x){
    summary(x)$call %>% as.character() %>% .[2]
  }) %>% unlist()
...
lmedat <- h2 %>% 
  filter(!is.na(region)) %>% 
  mutate(region = as.factor(region), trip = as.numeric(trip)) %>% 
  arrange(seal, num.from.tip)

## 13C 
m0 <- glm(c13 ~ region*year +region*state, data = lmedat) #do for n15 tmr
m1<- lme4::lmer(c13 ~ region*year + region*state + (1|seal), data = lmedat)
anova(m1, m0) # yes keep seal random effect
anova(m1)
drop1(m1, test="Chisq")
m2 <- update(m1, .~. - region:year)
drop1(m2, test="Chisq")
m3 <- update(m2, .~. -region:state)
drop1(m3, test="Chisq")
m4 <- update(m3, .~. -state)
drop1(m4, test="Chisq")
m5 <- update(m4, .~. -year)
...
## 15N
mm0 <- glm(n15 ~ region*year + region*state, data = lmedat)
mm1<- lme4::lmer(n15 ~ region*year + region*state + (1|seal), data = lmedat)
anova(mm1, mm0) # yes keep seal random effect
drop1(mm1, test="Chisq")
mm2 <- lme4::lmer(n15 ~ region*year + state + (1|seal), data = lmedat)
...
## Save results 
tab <- AICtab(m0,m1,m2,m3,m4,m5,  models = list(m0,m1,m2,m3,m4,m5)) 
tab <- AICtab(mm0,mm1,mm2,  models = list(mm0,mm1,mm2)) %>% 
  bind_rows(tab)
                
write.xlsx(tab, file="./output/Results_all.xlsx",
sheetName="LMER-AICSelection", append=TRUE)
tab <- rbind(tidy(m5), tidy(mm2)) %>% 
  mutate_if(is.numeric, ~round(., 2))
write.xlsx(tab, file="./output/Results_all.xlsx",
           sheetName="LMER-FinalFit", append=TRUE)
# write.csv(rbind(tidy(m2), tidy(mm1)), './output/GLM_final_fit.csv') # old code
...
p1 <- plot_model(m5, type = 'eff', terms = 'region', show.ci = TRUE) + 
  # lims(y = c(-16.1 , -15.65)) + 
  mytheme + 
...
p3 <- plot_model(mm2, type = 'eff', grid.breaks = 0, terms = 'state', colors = c('black', 'black')) + 
  mytheme + 
  labs(x = 'State', y = expression(paste( delta ^"15", "N", " (‰)")), title = NULL)

 p4 <- plot_model(mm2, type = 'int', grid = T, colors = c('black', 'black'), se = T) + 
  mytheme + 
  labs(x = 'Habitat', y = expression(paste( delta ^"15", "N", " (‰)")), title = NULL)

pp1 <- ggarrange(p1, p3, ncol = 3, labels = c('a', 'b'), widths = c(1,1,0.1))


pdf(file = './plots/SI_vs_region_year_state.pdf', height = 6, width = 6)
ggarrange(pp1, p4, nrow = 2, labels = c('', 'c'))
dev.off()
...
# pdf('./plots/mclust_biplot_spatialmap.pdf', width = 7, height = 10)
ggarrange(p1, p2, nrow = 2, labels = c('a', 'b'))
# dev.off()

## Proportion of SI values that were classified correctly 
## e.g. Group 1 and 3 = Oceanicl Group 2 = shelf 
data$correct.class = 0
data$correct.class[data$classification == '1' & data$region == 'oceanic'] <- 1
data$correct.class[data$classification == '3' & data$region == 'oceanic'] <- 1
data$correct.class[data$classification == '2' & data$region == 'shelf'] <- 1

sum(data$correct.class)/(data %>% filter(region != 'NA') %>% nrow)
...
library(DHARMa)
m0 <- glm(cluster~ yday*year, family = binomial, data = data_ocean)
m1 <- glmer(cluster ~ yday*year + (1|seal) , family = binomial, data = data_ocean)
anova(m1, m0)
drop1(m1, test = 'Chisq' )
m2 <- glmer(cluster ~ yday + year + (1|seal) , family = binomial, data = data_ocean)
AIC(m1, m2)
...
plot_model(m1, type = 'int', grid = T, colors = 'bw', show.loess = F) +
  mytheme + 
  labs(y = 'Probability of cluster 3' , x = 'Day of the year', title = NULL) 
dev.off()

tab <- tidy(m1) %>% 
  mutate_if(is.numeric, ~round(., 2))
write.xlsx(tab, file="./output/Results_all.xlsx",
           sheetName="OceanCluster x YDay", append=TRUE)
...
u <- data %>% filter(seal == '351') %>% dplyr::slice(1)
...
tmp <- data %>%
  filter(!is.na(trip)) %>% # n = 151 - only seals with gls data
  group_by(seal, trip) %>% 
  mutate(unique_class = length(unique(classification))) %>% # trips may have 2 or more segments but at most these segments belong to 2 unique classifications. 
  filter(unique_class == 1) %>% #n = 130
  mutate(state = ifelse(fs == 'unconstrained', 'unconstrained', 'provisioning')) %>% 
  ungroup() %>% 
  mutate(state = as.factor(state), yday = yday(date.est)) %>% 
  arrange(seal, trip) %>% 
  mutate(trip = as.character(trip)) %>% 
  filter(classification != '2') %>% 
  dplyr::select(seal, trip, yday, state, classification)  %>% 
  mutate(cluster = ifelse(classification == 1, 1, 0), cluster = as.factor(cluster))
 # remove cluster 2 since it has low number of samples. 
...
tmp <- tmp %>% 
  left_join(h1) 
tmp$yday <- scale(tmp$yday)[,1]

mod <- glmer(classification ~  t.timespent * yday +(1|seal), family = 'binomial', data = tmp)
fittedModel <- mod
simulationOutput <- simulateResiduals(fittedModel = fittedModel, n = 1000)
plot(simulationOutput)
testResiduals(simulationOutput)
...
# write.csv(tmp, './output/individual_isotope_summary.csv') #old code
write.xlsx(tmp, file="./output/Results_all.xlsx",
           sheetName="", append=TRUE)
...
p1 <- smap.dat %>%
  ...
  labs(x = NULL, y = 'Latitude (degrees)')
